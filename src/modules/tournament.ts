...
export const applyStats = (tournament: TournamentEntry) => {
...
export const applyTimings = (stats: { a: Stat[]; b: Stat[] }) => {
...
export const setCheckboxes = (size: 4 | 5 | 6) => {
  const teams = getTeams()
  if (teams === null) return
  teams.forEach((team) => {
    $all("tr[class]", team).forEach((player, i) => {
      ...
      if (i === 0) {
        if (!$<HTMLInputElement>("input", playerTd[3])?.checked) $<HTMLInputElement>("input", $all("td", player)[3])?.click()
      }
      if (i < size) {
        const [td0, td2] = [$<HTMLInputElement>("input", playerTd[0]), $<HTMLInputElement>("input", playerTd[2])]
        if (!td0?.checked) $<HTMLInputElement>("input", $all("td", player)[0])?.click()
        if (!td2?.checked) $<HTMLInputElement>("input", $all("td", player)[2])?.click()
      }
    })
  })
}